...
// 유스케이스 Provider
final getBusinessLocationsUseCaseProvider =
    Provider<GetBusinessLocationsUseCase>((ref) {
      final repository = ref.watch(businessRepositoryProvider);
      return GetBusinessLocationsUseCase(repository);
    });

final addBusinessLocationUseCaseProvider = Provider<AddBusinessLocationUseCase>(
  (ref) {
    final repository = ref.watch(businessRepositoryProvider);
    return AddBusinessLocationUseCase(repository);
  },
);

// 뷰모델 Provider
final businessSelectionViewModelProvider =
    StateNotifierProvider<BusinessSelectionViewModel, BusinessSelectionState>((
      ref,
    ) {
      final useCase = ref.watch(getBusinessLocationsUseCaseProvider);
      final addUseCase = ref.watch(addBusinessLocationUseCaseProvider);
      return BusinessSelectionViewModel(useCase, addUseCase);
    });
